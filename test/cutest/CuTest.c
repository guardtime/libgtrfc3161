...
{
	char* newStr = (char*) malloc( sizeof(char) * (unsigned)size );
	return newStr;
}
...
void CuStringInit(CuString* str)
{
	str->length = 0;
	str->size = STRING_MAX;
	str->buffer = (char*) malloc(sizeof(char) * (unsigned)str->size);
	str->buffer[0] = '\0';
}
...
{
	str->buffer = (char*) realloc(str->buffer, sizeof(char) * (unsigned) newSize);
	str->size = newSize;
}
...
void CuStringInsert(CuString* str, const char* text, int pos)
{
	...
	if (str->length + length + 1 >= str->size)
		CuStringResize(str, str->length + length + 1 + STRING_INC);
	memmove(str->buffer + pos + length, str->buffer + pos, (unsigned)(str->length - pos) + 1);
	str->length += length;
	memcpy(str->buffer + (unsigned)pos, text, (unsigned)length);
}
...
void CuFail_Line(CuTest* tc, const char* file, int line, const char* message2, const char* message)
{
	...
	if (tc->failCallback) tc->failCallback();
...
void CuSuiteAdd(CuSuite* testSuite, CuTest *testCase)
{
	...
	if (testSuite->failCallback != NULL) testCase->failCallback = testSuite->failCallback;
